...
describe("LED operations", () => {
	...
	describe("Headlight LED Commands", () => {
		test("should parse left headlight LED command", () => {
			// Test setting the left headlight to an RGB value
			const code = "rgbLed.set_left_headlight(10, 20, 30);"
			const bytecode = CppParser.cppToByte(code)

			// Verify bytecode structure
			expect(bytecode[0]).toBe(BytecodeOpCode.SET_LED)
			expect(bytecode[1]).toBe(LedID.LEFT_HEADLIGHT)
			expect(bytecode[2]).toBe(10) // Red
			expect(bytecode[3]).toBe(20) // Green
			expect(bytecode[4]).toBe(30) // Blue

			// Should end with END opcode
			expect(bytecode[5]).toBe(BytecodeOpCode.END)
		})

		test("should parse right headlight LED command", () => {
			// Test setting the right headlight to a different RGB value
			const code = "rgbLed.set_right_headlight(40, 50, 60);"
			const bytecode = CppParser.cppToByte(code)

			// Verify bytecode structure
			expect(bytecode[0]).toBe(BytecodeOpCode.SET_LED)
			expect(bytecode[1]).toBe(LedID.RIGHT_HEADLIGHT)
			expect(bytecode[2]).toBe(40) // Red
			expect(bytecode[3]).toBe(50) // Green
			expect(bytecode[4]).toBe(60) // Blue

			// Should end with END opcode
			expect(bytecode[5]).toBe(BytecodeOpCode.END)
		})

		test("should parse left and right headlight commands together", () => {
			// Test setting both headlights in sequence
			const code = `
      rgbLed.set_left_headlight(15, 25, 35);
      rgbLed.set_right_headlight(45, 55, 65);
    `
			const bytecode = CppParser.cppToByte(code)

			// Verify left headlight instruction
			expect(bytecode[0]).toBe(BytecodeOpCode.SET_LED)
			expect(bytecode[1]).toBe(LedID.LEFT_HEADLIGHT)
			expect(bytecode[2]).toBe(15) // Red
			expect(bytecode[3]).toBe(25) // Green
			expect(bytecode[4]).toBe(35) // Blue

			// Verify right headlight instruction
			expect(bytecode[5]).toBe(BytecodeOpCode.SET_LED)
			expect(bytecode[6]).toBe(LedID.RIGHT_HEADLIGHT)
			expect(bytecode[7]).toBe(45) // Red
			expect(bytecode[8]).toBe(55) // Green
			expect(bytecode[9]).toBe(65) // Blue

			// Should end with END opcode
			expect(bytecode[10]).toBe(BytecodeOpCode.END)
		})

		test("should parse headlight commands in if-else blocks", () => {
			// Test using headlights in control structures
			const code = `
			  if (10 > 5) {
				rgbLed.set_left_headlight(100, 0, 0);
			  } else {
				rgbLed.set_right_headlight(0, 100, 0);
			  }
			`
			const bytecode = CppParser.cppToByte(code)

			// Find the LED instructions in the bytecode
			let leftHeadlightIndex = -1
			let rightHeadlightIndex = -1

			for (let i = 0; i < bytecode.length; i += 5) {
			  if (bytecode[i] === BytecodeOpCode.SET_LED) {
					if (bytecode[i + 1] === LedID.LEFT_HEADLIGHT) {
				  leftHeadlightIndex = i
					} else if (bytecode[i + 1] === LedID.RIGHT_HEADLIGHT) {
				  rightHeadlightIndex = i
					}
			  }
			}

			// Both should be found in the bytecode (different branches)
			expect(leftHeadlightIndex).toBeGreaterThan(0)
			expect(rightHeadlightIndex).toBeGreaterThan(0)

			// Left headlight should be set to red
			expect(bytecode[leftHeadlightIndex + 2]).toBe(100) // Red
			expect(bytecode[leftHeadlightIndex + 3]).toBe(0)   // Green
			expect(bytecode[leftHeadlightIndex + 4]).toBe(0)   // Blue

			// Right headlight should be set to green
			expect(bytecode[rightHeadlightIndex + 2]).toBe(0)   // Red
			expect(bytecode[rightHeadlightIndex + 3]).toBe(100) // Green
			expect(bytecode[rightHeadlightIndex + 4]).toBe(0)   // Blue

			// Check that left headlight comes before right headlight in the bytecode
			// (since it's in the "if" branch which comes first)
			expect(leftHeadlightIndex).toBeLessThan(rightHeadlightIndex)
		  })

		test("should parse headlight commands with full brightness", () => {
			// Test maximum brightness values
			const code = "rgbLed.set_left_headlight(255, 255, 255);"
			const bytecode = CppParser.cppToByte(code)

			expect(bytecode[0]).toBe(BytecodeOpCode.SET_LED)
			expect(bytecode[1]).toBe(LedID.LEFT_HEADLIGHT)
			expect(bytecode[2]).toBe(255) // Max red
			expect(bytecode[3]).toBe(255) // Max green
			expect(bytecode[4]).toBe(255) // Max blue
		})

		test("should parse headlight commands with zero brightness", () => {
			// Test minimum brightness values (off)
			const code = "rgbLed.set_right_headlight(0, 0, 0);"
			const bytecode = CppParser.cppToByte(code)

			expect(bytecode[0]).toBe(BytecodeOpCode.SET_LED)
			expect(bytecode[1]).toBe(LedID.RIGHT_HEADLIGHT)
			expect(bytecode[2]).toBe(0) // No red
			expect(bytecode[3]).toBe(0) // No green
			expect(bytecode[4]).toBe(0) // No blue
		})
	})
